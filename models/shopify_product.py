...
class ProductShopify(models.Model):
    _name = 'product.shipify'
    _description = 'Product Import shipify'
...
class Product(models.Model):
    ...
    def create_product(self, p_item, instance_id):
        ...
        product_shopify_id = self.env['product.shipify'].sudo().search([('shopify_id', '=', p_item.id)],limit=1)
        if not product_shopify_id:
            vale={}
            vale['shopify_name'] = p_item.title if p_item.title else '' 
            vale['shopify_id'] = p_item.id if p_item.id else ''
            #vale['shopify_regular_price'] = p_item.id if p_item.id else ''
            #vale['shopify_sale_price'] = p_item.id if p_item.id else ''
            vale['shopify_product_status'] = p_item.status
            #vale['shopify_barcode'] = p_item.status
            #vale['shopify_sku'] = p_item.status
            vale['shopify_product_scope'] = p_item.published_scope if p_item.published_scope else ''
            #vale['shopify_product_weight'] = 
            #vale['shopify_product_qty'] = 
            vale['shopify_instance_id'] = self.env['shopify.instance']._context.get('shopify_instance_id')
            self.env['product.shipify'].create(vale)
        ##########
        #dict_p['active'] = False
            ...
            for variant in p_item.variants:
                ...
                if variant.option1 == 'Default Title' and product:
                    product.sudo().write({
                        ...
                        ###
                        'shopify_sale_price':variant.price,
                    })
    ...
    def export_product_stock_to_shopify(self,shopify_instance_id, p_id):
        ...
        for inv_data in self.get_inventory_level(shopify_instance_id, p_id):
            ...
            try:  
                shopify_qty=int(inv_data['available'])
                odoo_qty=int(inv_data['qty_available'])  
                change=odoo_qty-shopify_qty
                shopify.InventoryLevel.set(inv_data['location_id'], inv_data['inventory_item_id'], int(inv_data['available']) +change )#+ int(inv_data['change_qty'])
                #raise UserError( str(inv_data)+"/"+str( int(inv_data['change_qty'])) )
                _logger.info("\n\n\n\n new_quantity: %s    qty_available Qty: %s \n\n\n\n %s   change %s" % (inv_data['available'], inv_data['change_qty'],inv_data,change))
                success = True
...
class ProductImportTracking(models.Model):
    ...
    def create_or_update_record(self, vals):
        ...
        if existing_record:
            ...
            return super(ProductImportTracking, self).create({
                'since_id': '0',
                'is_completed': False,
                'status': 'start',
                'time': vals['time']
    ...
    def clear_all_rows(self):
        ...
        self.env['product.import.tracking'].search([]).unlink()
        _logger.info('\n\n\n\n clear_all_rows \n\n\n\n\n')
